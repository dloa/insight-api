...
function PeerSync(opts) {
  ...
  this.forceRpc = (config.currency == 'btc' ? false : true);
  this.load_peers();
...
PeerSync.prototype.handleBlock = function(info) {
    ...
    console.log("[p2p_sync] handleBlock")
    if(this.forceRpc){
        self.historicSync.start({forceRPC:1, broadcast:true}, function(){
        ...
        console.log("handleBlock: %s, prev: %s", blockHash, block.prev_hash)
...
PeerSync.prototype.run = function() {
  ...
  this.peerman = new PeerManager(self.pmConfig);
  ...
  this.peerman.on('netDisconnected', function() {
    logger.info("PeerSync disconnected")
