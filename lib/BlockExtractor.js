...
var  bitcore  = require('bitcore'),
     ...
     glob     = require('glob'),
     async    = require('async'),
     config   = imports.config || require('../config/config'),
...
function BlockExtractor(dataDir, network) {
  ...
  this.network = network === 'testnet' ? networks[config.currency].testnet: networks[config.currency].livenet;
...
BlockExtractor.prototype._getMagic = function() {
  ...
  //info("%s\n", this.currentParser.subject.toString('hex', 0, 50))  
  if (magic !=='00000000' && magic !== this.magic) {
      console.error("magic rx: %s, error count %d", magic, this.errorCount)  
