...
BlockDb.prototype._changeBlockHeight = function(hash, height, cb) {
  ...
  logger.log('etting TXS FROM %s to set it Main, height %s', hash, height);
...
BlockDb.prototype.fromHashWithInfo = function(hash, cb) {
  ...
  Rpc.getBlock(hash, function(err, info) {
    if (err || !info) {
        console.log('fromHashWithInfo cannot find block %s, error: %s', hash, err ? JSON.stringify(err) : '')
        return cb(err);
    }
    //console.log('fromHashWithInfo %s, %s', hash, JSON.stringify(info))
