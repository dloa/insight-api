...
function HistoricSync(opts) {
  ...
  this.network = config.network === 'testnet' ? networks[config.currency].testnet: networks[config.currency].livenet;
...
HistoricSync.prototype.getBlockFromRPC = function(cb) {
  ...
  info("updateStartBlock: start with tip: %s, height %s", tip, height)
  var blockInfo;
...
HistoricSync.prototype.checkNetworkSettings = function(next) {
  ...
  self.rpc.getBlockHash(0, function(err, res){
    if (!err && ( res && res.result !== self.genesis)) {
      info("got genesis %s, should be %s", res.result, self.genesis)	
...
HistoricSync.prototype.updateStartBlock = function(opts, next) {
  ...
  if (opts.startAt) {
    ...
    self.sync.bDb.getTip(function(err,tip, height) {
      ...
      async.doWhilst(
        function(cb) {
		  info("updateStartBlock: tip: %s, old tip %s", tip, oldtip)
          self.sync.bDb.fromHashWithInfo(tip, function(err, bi) {
            if(err){
                error("updateStartBlock: cannot get block from tip %s", tip)
                return cb(err)
            }		  
            blockInfo = bi ? bi.info : {};
			//info("updateStartBlock %s", JSON.stringify(blockInfo))
            if (oldtip)
        ...
        function(err) {
		  ...
		  info("Height: %s/%d", self.blockChainHeight, blockInfo.height)
...
HistoricSync.prototype.prepareFileSync = function(opts, next) {
  ...
  info("prepareFileSync height %s, blockChainHeight %s", self.height, self.blockChainHeight);  
  ...
  self.sync.bDb.getLastFileIndex(function(err, idx) {
	if(err) {
      info("prepareFileSync cannot get file index: %s", JSON.stringify(err));
    }

	info("prepareFileSync last file index %s", idx);
